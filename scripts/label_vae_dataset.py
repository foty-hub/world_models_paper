...
def main(vae_name: str = "vae", data_name: str = "random_data"):

    data_dir = Path("data") / data_name

    ckpt_dir = Path("experiments") / "vae" / vae_name
    ...
    mus     = z.create_array(f"latents/{vae_name}/mu",     shape=(0, 1001, 32), chunks=(1, 1001, 32), dtype=np.float32, overwrite=True)
    logvars = z.create_array(f"latents/{vae_name}/logvar", shape=(0, 1001, 32), chunks=(1, 1001, 32), dtype=np.float32, overwrite=True)
    # fmt: on
